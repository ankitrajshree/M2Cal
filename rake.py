"""
This is the base Rapid Automatic Keyword Extraction implementation with modification done to address the keyword
extraction in emails abstract/summary .
"""
...
class Rake(object):
    def __init__(self, stop_words_path="data/stoplists/SmartStoplist.txt", min_char_length=1, max_words_length=5, min_keyword_frequency=1,
                 min_words_length_adj=1, max_words_length_adj=1, min_phrase_freq_adj=2):
    ...
    def run(self, text, threshold):
        ...
        sorted_keywords_threshold = [keyword for keyword in sorted_keywords if keyword[1] > threshold]
        return sorted_keywords_threshold
...
#     text = "The pervasive use of the Web has connected billions of people all around the globe and enabled them to obtain information at their fingertips. This results in tremendous amounts of user-generated data which make users traceable. On the one hand, users can become vulnerable to potential risks ranging from persecution by governments to targeted frauds. Therefore, it is necessary for users to be able to safeguard their privacy without leaving their unnecessary traces of online activities. On the other hand, this information of traces is crucial for online vendors to provide personalized services and a lack of it would result in deteriorating quality of online personalization services. This leads to a dilemma of privacy and utility. This proposal investigates if an optimal trade-off between privacy and utility can be achieved considering the aforementioned dilemma. First, we show that new privacy risks in user-generated data can originate from innate heterogeneity of data. The results posit the requirement for striking the balance between preserving user data utility and protecting user privacy. Second, we propose a rigorous solution that searches for the optimal trade-off between privacy and utility. The proposed solution takes advantage of user behavioral patterns from social media to infer what and how much additional data is required to improve user privacy while keeping the utility of the resultant data. We next quantify the vulnerability of the resultant data against de-anonymization attacks. Our finding paves the way for generating untraceable user data to make de-anonymization attacks ineffective."
#
